...
function printStringLiteral(path, options) {
    ...
    const quoteCharacter = options_1.getStringQuotemark(options);
    const otherQuote = options_1.getAlternativeStringQuotemark(options);
...
function printNodeNoParens(path, options, print) {
    ...
    switch (node.type) {
        ...
        case 'ReturnStatement':
            return docBuilder_1.concat(['return ', docBuilder_1.join(', ', path.map(print, 'arguments'))]);
        case 'WhileStatement':
            parts.push('while ');
        ...
        case 'AssignmentStatement':
            ...
            if (node.init.length) {
                parts.push(' = ');
                parts.push(docBuilder_1.join(', ', path.map(print, 'init')));
            }
            return docBuilder_1.group(docBuilder_1.concat(parts));
        case 'CallStatement':
